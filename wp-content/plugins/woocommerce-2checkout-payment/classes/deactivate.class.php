...
class TWOCO_Deactivate {
    
    
    public function __construct() {
    
        $twoco_dir_name  = basename(dirname(__DIR__));
        $twoco_basename  = "{$twoco_dir_name}/woocommerce-2checkout-payment.php";
        add_filter( "plugin_action_links_{$twoco_basename}", array($this, 'twoco_deactivate_link'), 99);
        
        add_action('admin_enqueue_scripts', array($this, 'load_script'));
        
        add_action('admin_footer', array($this, 'deactivate_scripts'));
        add_action('wp_ajax_twoco_submit_uninstall_reason', array($this, "send_uninstall_reason"));
    }
    
    function twoco_deactivate_link($links) {
        
        
        if (array_key_exists('deactivate', $links)) {
            $links['deactivate'] = str_replace('<a', '<a class="twoco-deactivate-link"', $links['deactivate']);
        }
    ...
    public function deactivate_scripts() {
        ...
        <div class="twoco-deactivate-modal" id="twoco-deactivate-modal">
            <div class="twoco-deactivate-modal-wrap">
                <div class="twoco-deactivate-modal-header">
                    <h3><?php _e('Please Help Us to Make it Better for You.', 'product-import-export-for-woo'); ?></h3>
                </div>
                <div class="twoco-deactivate-modal-body">
                    <ul class="reasons">
                ...
                <div class="twoco-deactivate-modal-footer">
        ...
        <style type="text/css">
            ...
            .twoco-deactivate-modal.modal-active {display: block;}
            .twoco-deactivate-modal-wrap {
                width: 50%;
                position: relative;
                margin: 5% auto;
                background: #fff;
            }
            .twoco-deactivate-modal-header {
                border-bottom: 1px solid #eee;
                padding: 8px 20px;
            }
            .twoco-deactivate-modal-header h3 {
                line-height: 150%;
                margin: 0;
            }
            .twoco-deactivate-modal-body {padding: 5px 20px 20px 20px;}
            .twoco-deactivate-modal-body .input-text,.twoco-deactivate-modal-body textarea {width:75%;}
            .twoco-deactivate-modal-body .reason-input {
                margin-top: 5px;
                margin-left: 20px;
            }
            .twoco-deactivate-modal-footer {
    ...
    function load_script($hook) {
        ...
        // Preloader script
        wp_enqueue_script('twoco-deactivate', TWOCO_URL."/js/twoco-deactivate.js", array('jquery'), TWOCO_VERSION, true);
    }
